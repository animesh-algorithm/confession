...
const ConfessionsActions = ({
  ...
  const likeButton = () => (
    <i
      className={`${
        !confession.likes?.includes(auth.uid) ? "far" : "fas"
      } fa-heart`}
    >
      {" "}
  ...
  const commentButton = () => (
    <i
      className={`${
        !confession.commentsFrom?.includes(auth.uid) ? "far" : "fas"
      } fa-comment`}
    >
      {" "}
        ...
        <LinkContainer to="#">
          {confession.userId === "XBODMyuxsjQCw7LDM6ivYt0Atqq1" ? (
            ...
            <Card.Link
              ...
              onClick={() => unfollowUser(confession.userId)}
            >
              Unfollow
            </Card.Link>
          ) : (
